...
import com.makeurpicks.domain.Authority;
...
public class AuthServerApplication extends WebMvcConfigurerAdapter implements CommandLineRunner {
	...
	@Value("${config.oauth2.ui-uri:http://localhost:8080/}")
	private String ui;

	@Value("${config.oauth2.admin-uri:http://localhost:9000/admin/}")
	...
	protected static class SecurityConfig extends WebSecurityConfigurerAdapter {
		...
		protected void configure(HttpSecurity http) throws Exception {
			http.formLogin().and()
					...
					// disable anonymous
					.anonymous().disable()
					// authorize+ authenticate the requests
					.authorizeRequests().anyRequest().authenticated();
		}

		@Override
		public void configure(WebSecurity web) throws Exception {
			// ignore all with patter /players/**
	...
	protected static class AuthorizationServerConfig extends AuthorizationServerConfigurerAdapter {
		...
		@Value("${config.oauth2.privateKey:private_default}")
		private String privateKey;

		@Value("${config.oauth2.publicKey:public_default}")
		...
		public void configure(ClientDetailsServiceConfigurer clients) throws Exception {
			// looks for the clients in DB rather than in memory
	...
	public void run(String... args) throws Exception {
		PlayerBuilder p = new PlayerBuilder("prithvish", "prithvish.mukherjee@techolution.com", "secret");
		p.adAdmin();
		Player player = p.build();
